...
The CLI is the only part of the compiler that uses an external library, [`Spectre.Console`](https://spectreconsole.net/e) to parse out arguments, options and commands. The decision to use a library for this was not made lightly however there are a lot of subtle edge cases when it comes to providing a clean and intuitive CLI experience and using a library that has been battle tested and develop on it's own for years was the best option. The library is also rather small and extremely specific to the task so it doesn't add a significant amount of bloat to the project.

The CLI is implemented in `decaf/Main.cs`.
